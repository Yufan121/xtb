...
module xtb_xtb_repulsion
   ...
   interface repulsionEnGrad
      ...
      module procedure :: repulsionEnGrad_latpPerAtom
      module procedure :: repulsionEnGrad_neighsPerAtom
...
!> Lattice point based implementation of the repulsion energy
subroutine repulsionEnGrad_latpPerAtom(mol, repData, repDataPerAtom, trans, cutoff, energy, gradient, &
      & sigma)
   ...
   !> Repulsion parametrisation
   type(TRepulsionData), intent(in) :: repData, repDataPerAtom     ! TODO
   ...
   !$acc enter data create(energies, rij, dS, dG) copyin(gradient, sigma, mol, mol%at, &
   !$acc& mol%xyz, repData, repData%alpha, repData%zeff, repDataPerAtom, repDataPerAtom%alpha, repDataPerAtom%zeff, trans)
...
#else
   !$omp parallel do default(none) reduction(+:energies, gradient, sigma) &
   !$omp shared(repData, repDataPerAtom, mol, cutoff2, trans) &
   !$omp private(iat, jat, itr, iZp, jZp, r2, rij, r1, alpha, zeff, kexp, &
   !$omp& t16, t26, t27, dE, dG, dS, k, l)
#endif
   do iAt = 1, mol%n ! Yufan: TODO
      do jAt = 1, mol%n
         ...
         iZp = mol%at(iAt)
         jZp = mol%at(jAt)
         alpha = sqrt((repData%alpha(iZp) + repDataPerAtom%alpha(iAt))*(repData%alpha(jZp) + repDataPerAtom%alpha(jAt)))      ! TODO
         zeff = (repData%zeff(iZp) + repDataPerAtom%zeff(iAt))*(repData%zeff(jZp) + repDataPerAtom%zeff(jAt))               ! TODO
         if (iZp > 2 .or. jZp > 2) then
            kExp = repData%kExp
         else
            kExp = repData%kExpLight
   ...
   !$acc exit data copyout(energies, gradient, sigma) delete(mol, mol%at, &
   !$acc& mol%xyz, repData, repData%alpha, repData%zeff, repDataPerAtom, repDataPerAtom%alpha, repDataPerAtom%zeff, rij, dG, dS, trans)
#endif

   energy = energy + sum(energies)

end subroutine repulsionEnGrad_latpPerAtom
...
!> Lattice point based implementation of the repulsion energy
subroutine repulsionEnGrad_neighsPerAtom(mol, repData, repDataPerAtom, neighs, neighList, energy, &
      & gradient, sigma)
   ...
   !> Repulsion parametrisation
   type(TRepulsionData), intent(in) :: repData, repDataPerAtom
   ...
   !$omp parallel do default(none) reduction(+:energies, gradient, sigma) &
   !$omp shared(repData, repDataPerAtom, mol, neighs, neighList) &
   !$omp private(iat, jat, ij, img, iZp, jZp, r2, rij, r1, alpha, zeff, kexp, &
   !$omp& t16, t26, t27, dE, dG, dS)
   do iAt = 1, len(mol)
      iZp = mol%at(iAt)
...
end subroutine repulsionEnGrad_neighsPerAtom
